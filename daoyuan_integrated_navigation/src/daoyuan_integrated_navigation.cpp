...
{
    INS570D::INS570D()
        : private_nh("~"), init_flag(false), length(58), data_packet_start(0), 
                           relativeX(0), relativeY(0), relativeZ(0), 
                           LOOP_RATE(100), 
                           Roll(0), Pitch(0), Yaw(0), 
                           Latitude(0), Longitude(0), Altitude(0), 
                           w_Z(0), Ve(0), Vn(0), Vg(0), l(0)
    {
        initForROS();       //ROS初始化
        init_open_serial(); //
        write_path.open(file_path, std::ios::trunc);
    }
    ...
    ***********************************************************************************************/
    ...
    void INS570D::publish_control()
    {
        ...
        try
        {
            ...
            {
                ROS_WARN("relative position error");
                return;
            }
            else
            {
        ...
        if (sflag || (!write_path.eof() && count > 10 && sqrt(pow(l_pose.pose.position.x - current_pose.pose.position.x, 2) + pow(l_pose.pose.position.y - current_pose.pose.position.y, 2)) > 1.5))
        {
            write_path << double(current_pose.pose.position.x) << " " << double(current_pose.pose.position.y) << " " << double(current_pose.pose.position.z) << std::endl;
            count = 0;
            sflag = false;
        ...
        acc_pub.publish(acc);
        std::cout << "111" << current_pose << std::endl
                  << "222" << velocity << std::endl
                  << "333" << acc << std::endl;
    }
